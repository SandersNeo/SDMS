...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Параметры.РасширенныйОтбор) = Тип("Массив") 
		И Параметры.РасширенныйОтбор.Количество() > 0 Тогда
		
		Для Каждого ОписаниеОтбора Из Параметры.РасширенныйОтбор Цикл
			НовыйЭлемент = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			НовыйЭлемент.ЛевоеЗначение    = Новый ПолеКомпоновкиДанных(ОписаниеОтбора["Поле"]);
			НовыйЭлемент.ВидСравнения     = ОписаниеОтбора["ВидСравнения"];
			НовыйЭлемент.Использование    = ОписаниеОтбора["Использование"];
			НовыйЭлемент.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный;
			
			Если ОписаниеОтбора["Значение"] <> Неопределено Тогда
				НовыйЭлемент.ПравоеЗначение = ОписаниеОтбора["Значение"];
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОткрытьФормуНастроекДинамическогоСписка(Команда)
	...
	КлючХранилища = "Справочник.Проекты.Форма.ФормаСписка.Список";
...
Процедура ОбновлениеПользовательскихНастроекДинамическогоСписка()
	...
	ОбщегоНазначенияВызовСервера.ПользовательскиеНастройкиДинамическогоСписка(
		"Справочник.Проекты.Форма.ФормаСписка.Список", ВариантыНастроек, ЭтотОбъект,
...
Процедура ПрименениеНастройкиДинамическогоСписка(Знач Идентификатор)
	...
	ОбщегоНазначенияВызовСервера.ПрименитьНастройкуДинамическогоСпискаНаСервере(
		"Справочник.Проекты.Форма.ФормаСписка.Список", ВариантыНастроек, Идентификатор,
...
Функция НастройкиКолонокПоУмолчанию()
	...
	Состав.Добавить("Наименование", "Название", Ложь);
	Состав.Добавить("Автор", "Автор", Ложь);
	Состав.Добавить("Руководитель", "Руководитель", Ложь);
	Состав.Добавить("Заказчик", "Заказчик", Истина);
...
Функция ПолучитьКлючиНастроек()
	...
	Ключи.Вставить("Объект", "Справочник.Проекты.ФормаСписка");
